...
class_rf_wf <- workflow() %>% 
  ...
  add_recipe(pokemon_recipe)
param_rf_grid <- grid_regular(mtry(range = c(1, 8)), trees(range = c(50, 2150)), min_n(range = c(2, 16)), levels = 8)
```
...
- `trees`: Range from 50 to 2150 (adds 300 for every level)
...
The `roc_auc` of my best-performing random forest model on the folds is 0.7253002.
...
roc_auc_model <- c( 0.6287888, 0.7253002, 0.6991073)
...
The best-performing model is the random forest with `mtry = 2`, `trees = 1850`, and `min_n = 2`. It has an AUC value of 0.9999008 which is very high and indicates the model did a very good job.
